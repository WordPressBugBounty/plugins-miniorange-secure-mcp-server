<?php
/**
 * Execute callbacks for the core Categories ability pack.
 *
...
/**
 * Class Categories_Provider
 *
 * Static execute callbacks for category (taxonomy "category") abilities.
 */
class Categories_Provider {
	...
	public static function create( $input = array() ) {
		...
		if ( '' === $name ) {
			return new WP_Error( 'missing_name', __( 'A name is required to create a category.', 'mosmcp-abilities' ) );
		}
		...
		if ( $parent_id > 0 ) {
			...
			if ( ! $parent || is_wp_error( $parent ) ) {
				return new WP_Error( 'parent_not_found', __( 'No category found with the given parent_id.', 'mosmcp-abilities' ) );
			}
		}
		...
		if ( $existing ) {
			...
			return new WP_Error(
				'category_exists',
				sprintf(
					/* translators: 1: category name, 2: existing category ID. */
					__( 'A category named "%1$s" already exists (ID %2$d).', 'mosmcp-abilities' ),
					$name,
					$existing_id
				)
			);
	...
	/**
	 * Deletes a category (its posts move to the default category).
	 *
	 * @param array<string, mixed> $input Ability input.
	 * @return array<string, mixed>|WP_Error
	 */
	public static function delete( $input = array() ) {
		...
		if ( empty( $input['confirm'] ) || true !== $input['confirm'] ) {
			return new WP_Error( 'confirmation_required', __( 'Deleting a category cannot be undone. Set confirm to true to proceed.', 'mosmcp-abilities' ) );
		}

		$default_id = (int) get_option( 'default_category' );

		if ( (int) $term->term_id === $default_id ) {
			return new WP_Error( 'cannot_delete_default', __( 'The default category cannot be deleted.', 'mosmcp-abilities' ) );
		}
		...
		if ( true !== $result ) {
			return new WP_Error( 'delete_failed', __( 'The category could not be deleted.', 'mosmcp-abilities' ) );
		}
		...
		return array(
			...
			'name'           => $name,
			'deleted'        => true,
			'posts_moved_to' => ( $default && ! is_wp_error( $default ) ) ? (string) $default->name : '',
		);
	}

	/**
	 * Finds categories by name, or looks up a category by ID.
	 *
	 * @param array<string, mixed> $input Ability input.
	 * @return array<string, mixed>|WP_Error
	 */
	public static function find( $input = array() ) {
		...
		if ( $id <= 0 && '' === $search ) {
			return new WP_Error( 'missing_input', __( 'Provide either "search" (a category name) or "id" (a category ID).', 'mosmcp-abilities' ) );
		}
		...
		if ( $id > 0 ) {
			...
			return array(
				'showing' => 1,
				'total'   => 1,
				'matches' => array( self::format_category( $term ) ),
			);
		}
		...
		foreach ( $terms as $term ) {
			$matches[] = self::format_category( $term );
		}
	...
	/**
	 * Lists all categories, including empty ones.
	 *
	 * @param array<string, mixed> $input Ability input.
	 * @return array<string, mixed>|WP_Error
	 */
	...
	/**
	 * Lists the direct sub-categories of a category.
	 *
	 * @param array<string, mixed> $input Ability input.
	 * @return array<string, mixed>|WP_Error
	 */
	public static function list_children( $input = array() ) {
		$term = self::require_term( $input );
		if ( $term instanceof WP_Error ) {
			return $term;
		}
		...
		foreach ( $children as $child ) {
			$categories[] = self::format_category( $child );
		}

		return array(
			'showing'     => count( $categories ),
			'total'       => count( $categories ),
			'parent_name' => (string) $term->name,
			'categories'  => $categories,
		);
	}

	/**
	 * Lists categories that contain no posts.
	 *
	 * @param array<string, mixed> $input Ability input.
	 * @return array<string, mixed>|WP_Error
	 */
	public static function list_empty( $input = array() ) {
		...
		foreach ( $terms as $term ) {
			if ( 0 === (int) $term->count ) {
				$empty[] = self::format_category( $term );
			}
		}

		$page = array_slice( $empty, $offset, $per_page );
	...
	/**
	 * Lists posts in a category (including its sub-categories).
	 *
	 * @param array<string, mixed> $input Ability input.
	 * @return array<string, mixed>|WP_Error
	 */
	...
	/**
	 * Changes a category's description.
	 *
	 * @param array<string, mixed> $input Ability input.
	 * @return array<string, mixed>|WP_Error
	 */
	public static function update_description( $input = array() ) {
		...
		$result = wp_update_term( $term->term_id, 'category', array( 'description' => $description ) );
		if ( is_wp_error( $result ) ) {
			return $result;
		}

		$updated = get_term( $term->term_id, 'category' );
	...
	public static function update_name( $input = array() ) {
		...
		$result = wp_update_term( $term->term_id, 'category', array( 'name' => $name ) );
		if ( is_wp_error( $result ) ) {
			return $result;
		}
	...
	public static function update_slug( $input = array() ) {
		...
		if ( $existing && (int) $existing->term_id !== (int) $term->term_id ) {
			return new WP_Error(
				'slug_in_use',
				sprintf(
					/* translators: 1: slug, 2: category name. */
					__( 'The slug "%1$s" is already used by the category "%2$s".', 'mosmcp-abilities' ),
					$slug,
					$existing->name
				)
			);
		...
		$result = wp_update_term( $term->term_id, 'category', array( 'slug' => $slug ) );
		if ( is_wp_error( $result ) ) {
			return $result;
		}
	...
	/**
	 * Loads the target category term or returns a WP_Error.
	 *
	 * @param array<string, mixed> $input Ability input.
	...
	private static function require_term( $input ) {
		$id   = isset( $input['id'] ) ? absint( $input['id'] ) : 0;
		$term = $id > 0 ? get_term( $id, 'category' ) : null;
	...
	/**
	 * Formats one category term as the standard category item.
	 *
	...
	/**
	 * Admin edit URL for a category term.
	 *
	 * @param int $term_id Term ID.
	 * @return string
	 */
	private static function edit_url( $term_id ) {
		return (string) admin_url( 'term.php?taxonomy=category&tag_ID=' . $term_id );
	}
}
